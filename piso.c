...
/* Private port of ARK-4's vshctrl/isoreader.c for the xmb_gamecache
 * plugin: identical logic, but with its own state so it never races
 * VshCtrl's shared global ISO reader (used by its video virtualization
 * on other threads).  Decompressors come from firmware/SystemControl
 * exports; buffers are allocated once and kept. */
#include <string.h>
#include <pspsdk.h>
#include <pspkernel.h>
...
/* SystemCtrlForKernel exports (see imports.S) */
...
/* buffers are allocated once (user partition) and reused across opens */
static int isoAlloc(){
    SceUID id;
    if (ciso_dec_buf == NULL)     ciso_dec_buf = gc_alloc(DAX_BLOCK_SIZE + 64, &id);
    if (ciso_com_buf == NULL)     ciso_com_buf = gc_alloc(DAX_COMP_BUF + 64, &id);
    if (g_sector_buffer == NULL)  g_sector_buffer = gc_alloc(SECTOR_SIZE, &id);
    if (g_CISO_idx_cache == NULL) g_CISO_idx_cache = gc_alloc(4*CISO_IDX_MAX_ENTRIES, &id);
    if (ciso_dec_buf == NULL || ciso_com_buf == NULL ||
        g_sector_buffer == NULL || g_CISO_idx_cache == NULL)
        return -1;
    return 0;
}
...
int piso_open(const char *path)
{
    ...
    read_data = (int (*)(void *, u32, u32))&read_compressed_data;
...
int piso_total_sectors(void)
{
    return g_total_sectors;
}

void piso_close(void)
{
    int k1 = pspSdkSetK1(0);
...
int piso_getfileinfo(char * path, u32 *filesize, u32 *lba)
{
    int ret = 0;
    Iso9660DirectoryRecord rec;
    int k1 = pspSdkSetK1(0);
...
int piso_read(void *buffer, u32 lba, int offset, u32 size)
{
    int k1 = pspSdkSetK1(0);
